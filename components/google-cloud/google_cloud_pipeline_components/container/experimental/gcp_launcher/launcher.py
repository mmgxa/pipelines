...
def _parse_args(args):
  ...
  parser.add_argument(
      ...
      required=(parsed_args.type in {
          ...
          'BigqueryQueryJob', 'BigqueryCreateModel'
      }),
      default=argparse.SUPPRESS)
  parser.add_argument(
      '--output_info',
  ...
  parser.add_argument(
      ...
      required=(parsed_args.type in {'BigqueryQueryJob',
                                     'BigqueryCreateModel'}),
      default=argparse.SUPPRESS)
  parsed_args, _ = parser.parse_known_args(args)
...
def main(argv):
  ...
  if parsed_args['type'] == 'BigqueryCreateModel':
    bigquery_query_job_remote_runner.create_bigquery_model(**parsed_args)
