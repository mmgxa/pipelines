...
class BigqueryQueryJobRemoteRunnerUtilsTests(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self._executor_input_create_query_job = '{"outputs":{"artifacts":{"destination_table":{"artifacts":[{"metadata":{},"name":"foobar","type":{"schemaTitle":"google.BQTable"}}]}},"outputFile":"' + self._output_file_path + '"}}'
    self._executor_input_create_model = '{"outputs":{"artifacts":{"model":{"artifacts":[{"metadata":{},"name":"foobar","type":{"schemaTitle":"google.BQMLModel"}}]}},"outputFile":"' + self._output_file_path + '"}}'
  ...
  def test_create_query_job_succeeded(self, mock_time_sleep, mock_get_requests,
    ...
    bigquery_query_job_remote_runner.create_bigquery_job(
        ...
        self._executor_input_create_query_job)
  ...
  @mock.patch.object(requests, 'get', autospec=True)
  @mock.patch.object(time, 'sleep', autospec=True)
  def test_create_query_job_with_job_config_override_succeeded(
  ...
  @mock.patch.object(requests, 'get', autospec=True)
  @mock.patch.object(time, 'sleep', autospec=True)
  def test_create_query_job_poll_existing_job_succeeded(self, mock_time_sleep,
                                                        mock_get_requests, _,
                                                        mock_auth):
  ...
  @mock.patch.object(google.auth, 'default', autospec=True)
  @mock.patch.object(google.auth.transport.requests, 'Request', autospec=True)
  def test_create_query_job_check_job_exists_wrong_format(self, _, mock_auth):
    # Mimic the case that self._gcp_resources already stores the job uri.
    with open(self._gcp_resources, 'w') as f:
      f.write(
  ...
  @mock.patch.object(google.auth.transport.requests, 'Request', autospec=True)
  @mock.patch.object(requests, 'post', autospec=True)
  def test_create_query_job_failed_no_selflink(self, mock_post_requests, _,
                                               mock_auth):
    creds = mock.Mock()
    creds.token = 'fake_token'
    mock_auth.return_value = [creds, 'project']
  ...
  @mock.patch.object(requests, 'get', autospec=True)
  @mock.patch.object(time, 'sleep', autospec=True)
  def test_create_query_job_poll_existing_job_failed(self, mock_time_sleep,
                                                     mock_get_requests, _,
                                                     mock_auth):
    # Mimic the case that self._gcp_resources already stores the job uri.
    with open(self._gcp_resources, 'w') as f:
...
# Tests for create model
  ...
  @mock.patch.object(requests, 'get', autospec=True)
  @mock.patch.object(time, 'sleep', autospec=True)
  def test_create_model_succeeded(self, mock_time_sleep, mock_get_requests,
                                  mock_post_requests, _, mock_auth):
    creds = mock.Mock()
    creds.token = 'fake_token'
    mock_auth.return_value = [creds, 'project']
    ...
    bigquery_query_job_remote_runner.create_bigquery_model(
        self._job_type, self._project, self._location, self._payload,
        self._job_configuration_query_override, self._gcp_resources,
        self._executor_input_create_model)
    mock_post_requests.assert_called_once_with(
        url=f'https://www.googleapis.com/bigquery/v2/projects/{self._project}/jobs',
        data=(
            '{"configuration": {"query": {"query": "CREATE OR REPLACE MODEL bqml_tutorial.penguins_model OPTIONS (model_type=\'linear_reg\', input_label_cols=[\'body_mass_g\']) AS SELECT * FROM `bigquery-public-data.ml_datasets.penguins` WHERE body_mass_g IS NOT NULL", "useLegacySql": false}}, "jobReference": {"location": "US"}}'
  ...
  @mock.patch.object(requests, 'get', autospec=True)
  @mock.patch.object(time, 'sleep', autospec=True)
  def test_create_model_with_job_config_override_succeeded(
      self, mock_time_sleep, mock_get_requests, mock_post_requests, _,
      mock_auth):
    creds = mock.Mock()
    creds.token = 'fake_token'
  ...
  @mock.patch.object(requests, 'get', autospec=True)
  @mock.patch.object(time, 'sleep', autospec=True)
  def test_create_model_poll_existing_job_succeeded(self, mock_time_sleep,
                                                    mock_get_requests, _,
                                                    mock_auth):
    # Mimic the case that self._gcp_resources already stores the job uri.
    with open(self._gcp_resources, 'w') as f:
  ...
  @mock.patch.object(google.auth, 'default', autospec=True)
  @mock.patch.object(google.auth.transport.requests, 'Request', autospec=True)
  def test_create_model_check_job_exists_wrong_format(self, _, mock_auth):
    # Mimic the case that self._gcp_resources already stores the job uri.
    with open(self._gcp_resources, 'w') as f:
      f.write(
          '{"resources": [{"resourceType": "BigqueryQueryJob", "resourceUri": "https://www.googleapis.com/bigquery/v2/projects/test_project/jobs/fake_job_no_location"}]}'
  ...
  @mock.patch.object(google.auth.transport.requests, 'Request', autospec=True)
  @mock.patch.object(requests, 'post', autospec=True)
  def test_create_model_failed_no_selflink(self, mock_post_requests, _,
                                           mock_auth):
    creds = mock.Mock()
    creds.token = 'fake_token'
    mock_auth.return_value = [creds, 'project']
  ...
  @mock.patch.object(requests, 'get', autospec=True)
  @mock.patch.object(time, 'sleep', autospec=True)
  def test_create_model_poll_existing_job_failed(self, mock_time_sleep,
                                                 mock_get_requests, _,
                                                 mock_auth):
    # Mimic the case that self._gcp_resources already stores the job uri.
    with open(self._gcp_resources, 'w') as f:
  ...
  @mock.patch.object(requests, 'get', autospec=True)
  @mock.patch.object(time, 'sleep', autospec=True)
  def test_create_model_failed_no_query_result(self, mock_time_sleep,
                                               mock_get_requests,
  ...
  @mock.patch.object(requests, 'get', autospec=True)
  @mock.patch.object(time, 'sleep', autospec=True)
  def test_create_model_failed_not_create_model(self, mock_time_sleep,
                                                mock_get_requests,
                                                mock_post_requests, _,
                                                mock_auth):
    creds = mock.Mock()
